...
| `POST` | `/analysis/reports` | 生成并保存 LLM 分析报告 |
...
生成成功的报告以 UTF-8 Markdown 原子写入 `REPORT_STORAGE_DIR`，同一统计周期重复生成时保留独立版本。历史列表按生成时间倒序，可用成对的 `start_time`、`end_time` 筛选统计周期有交集的报告；历史文件永久保留且视为只读。
