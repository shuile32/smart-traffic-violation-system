...
def test_create_user_duplicate_409(db, seeded_roles):
    UserService(db).create_user(username="u1", password="p", phone=None, email=None, role_code="citizen")
...
def test_create_user_role_not_found_400(db, seeded_roles):
    with pytest.raises(Exception) as exc:
        UserService(db).create_user(username="u1", password="p", phone=None, email=None, role_code="bogus")
    assert exc.value.status_code == 400


def test_list_users_filter_by_status(db, seeded_roles):
    UserService(db).create_user(username="a", password="p", phone=None, email=None, role_code="citizen")
    u = UserService(db).create_user(username="b", password="p", phone=None, email=None, role_code="citizen")
...
def test_update_user_password_and_role(db, seeded_roles):
    u = UserService(db).create_user(username="u1", password="p", phone=None, email=None, role_code="citizen")
